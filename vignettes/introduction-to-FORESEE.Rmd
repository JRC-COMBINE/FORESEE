...
```r
library(FORESEE)
EnvForRunningYeoJohnsonTrans <- new.env()
EnvForRunningYeoJohnsonTrans$YeoJohnsonTrans <- YeoJohnsonTrans
tmp <- with(EnvForRunningYeoJohnsonTrans, {ForeseeTrain(TrainObject = GDSC,
...
If you check EnvForRunningYeoJohnsonTrans$Performance, you'll see the auc of roc curve has improved in comparison to 
...
```r
library(FORESEE)
EnvForMedianDupHandling <- new.env()
tmp <- with(EnvForMedianDupHandling, {ForeseeTrain(TrainObject = GDSC,
                                                TestObject = GSE6434, DrugName = "Docetaxel",
                                                GeneFilter = "variance", CellResponseType = "LN_IC50",
                                                CellResponseTransformation = "powertransform",
                                                DuplicationHandling = median);
  ForeseeTest(TestObject = TestObject, ForeseeModel = ForeseeModel)}
)
```
You can check EnvForMedianDupHandling$Performance to see how a median Duplication handling has performed.
...
```r
library(FORESEE)
EnvForDupHandling <- new.env()
EnvForDupHandling$Quan33 <- Quan33
tmp <- with(EnvForDupHandling, {ForeseeTrain(TrainObject = GDSC,
                                                TestObject = GSE6434, DrugName = "Docetaxel",
                                                GeneFilter = "variance", CellResponseType = "LN_IC50",
                                                CellResponseTransformation = "powertransform",
...
```
Again you can check EnvForDupHandling$Performance to see how a our own Duplication handler has performed.
...
```r
library(FORESEE)
EnvForHomogenization <- new.env()
EnvForHomogenization$BER <- BER
tmp <- with(EnvForHomogenization, {ForeseeTrain(TrainObject = GDSC,
                                                TestObject = GSE6434, DrugName = "Docetaxel",
                                                GeneFilter = "variance", 
                                                CellResponseType = "LN_IC50",
                                                CellResponseTransformation = "powertransform",
...
```r
library(FORESEE)
EnvForFilter <- new.env()
tmp <- with(EnvForFilter, {ForeseeTrain(TrainObject = GDSC,
                                                TestObject = GSE6434, DrugName = "Docetaxel",
                                                CellResponseType = "LN_IC50",
                                                CellResponseTransformation = "powertransform",
                                                DuplicationHandling = "first",
...
EnvForFilter$GDSCFiltered <- GDSCFiltered
tmp <- with(EnvForFilter, {ForeseeTrain(TrainObject = GDSCFiltered,
                                                TestObject = GSE6434, DrugName = "Docetaxel",
                                                CellResponseType = "LN_IC50",
                                                CellResponseTransformation = "powertransform",
                                                DuplicationHandling = "first",
...
library(FORESEE)
EnvForProc <- new.env()
EnvForProc$equalizer <- equalizer
tmp <- with(EnvForProc, {ForeseeTrain(TrainObject = GDSC,
                                                TestObject = GSE6434, DrugName = "Docetaxel",
                                                CellResponseType = "LN_IC50",
                                                CellResponseTransformation = "powertransform",
                                                DuplicationHandling = "first",
...
library(FORESEE)
EnvForBB <- new.env()
EnvForBB$GaussianWrapper <- GaussianWrapper
tmp <- with(EnvForBB, {ForeseeTrain(TrainObject = GDSC,
                                                TestObject = GSE6434, DrugName = "Docetaxel",
                                                CellResponseType = "LN_IC50",
                                                CellResponseTransformation = "powertransform",
                                                DuplicationHandling = "first",
