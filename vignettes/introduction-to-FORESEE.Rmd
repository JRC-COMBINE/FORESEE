...
# Erlotinib in GDSC
GDSC$DrugInfo$DRUG_NAME[grep("Erlotinib",GDSC$DrugInfo$DRUG_NAME,ignore.case=TRUE)]
# Erlotinib in CTRP
CTRP$DrugInfo$DRUG_NAME[grep("Erlotinib",CTRP$DrugInfo$DRUG_NAME,ignore.case=TRUE)]
```

In order to enable FORESEE to find the correct drug to model, we need to adjust the drug names to a common name.
Then, we can run FORESEE exactly like for the cell2patient case. 
Since the drug response in both platforms is given as AUC (continuous values), we pick pearson correlation as measure of evaluation.


```r
#Capitalize erlotinib in CTRP object
colnames(CTRP$AUC)[grep("Erlotinib",colnames(CTRP$AUC),ignore.case=TRUE)[1]]<-"Erlotinib"
...
Patiend-derived xenografts are often tested to obtain a better estimate of a drug response in patients than achievable with cell lines. FORESEE can create pdx2patient models to help determining the performance of those. In this example we check if the WITKIEWICZ pdx data [@xenograft2] can be used to train a model that predicts the drug response of GSE6434 breast cancer patients [@Chang2005].
...
WITKIEWICZ$DrugInfo$Item.Name[grep("Docetaxel",WITKIEWICZ$DrugInfo$Item.Name,ignore.case=TRUE)]
...
```r
#Remove the brand name of the drug in WITKIEWICZ pdx object
colnames(WITKIEWICZ$AUC)[grep("Docetaxel",colnames(WITKIEWICZ$AUC),ignore.case=TRUE)[1]]<-"Docetaxel"
